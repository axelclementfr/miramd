...
- **Double-clic pour focus** : double-cliquer dans le textarea source déplace la preview sur la position du curseur (`textareaEl.selectionStart`) avec un scroll smooth, et applique pendant ~1,6s une classe `.split-click-target` au bloc rendu le plus proche de la position cible (heading, paragraphe, liste, code, table, hr) — outline accent pulsant qui **entoure** l'élément (local, pas une barre full-width invasive). Single-click ne déclenche aucun sync visuel pour ne pas perturber l'écriture continue.
