...
- **Highlight du mot exact en double-clic** : `highlightWordInPreview()` extrait le mot via `selectionStart`..`selectionEnd`, compte son occurrence dans la source jusqu'au curseur, puis marque la même occurrence dans la preview via la **CSS Highlight API** (`Highlight` + `CSS.highlights.set('split-word-target', …)`). Fallback **Selection API** (`window.getSelection().addRange()`) si la Highlight API n'est pas dispo. Si le mot n'est pas trouvable (cross-boundary, syntaxe markdown vs rendu), fallback final sur l'outline du bloc (`flashClickTarget`).
- **No-rollback du scroll après sélection** : `suppressScrollSyncUntil = Date.now() + 500` après chaque double-clic. `handleScroll` est no-op pendant cette fenêtre — laisse le smooth scroll atterrir cleanly sans être interrompu par les events scroll auto-déclenchés (auto-scroll vers caret).
