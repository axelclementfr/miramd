...
- **Cleanup unifié des highlights** : `clearAllSplitHighlights()` est appelé en tête de chaque double-clic — clear le précédent word highlight (CSS Highlight + Selection si dans la preview) **et** l'outline du précédent bloc. Évite d'avoir deux sélections accentuées simultanément quand l'utilisateur double-clique rapidement.
