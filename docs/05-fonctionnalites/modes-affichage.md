...
- **Préservation du focus pendant l'écriture** : `muyaService.setMarkdown(value)` re-render le DOM du pane et vole le focus du textarea (WebKitGTK quirk). SourcePane sauvegarde `document.activeElement === textareaEl` + `selectionStart/End` + `scrollTop` avant l'appel, restaure immédiatement après et défère un microtask pour les éventuels focus shifts asynchrones de Muya.
