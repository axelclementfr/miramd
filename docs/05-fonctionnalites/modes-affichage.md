...
- **Sync différée pour préserver le focus** : `muyaService.setMarkdown(value)` re-render le DOM du pane et vole inévitablement le focus du textarea (WebKitGTK quirk, save/restore se sont avérés insuffisants). Solution : on **diffère le push tant que le textarea est focus**. SourcePane garde `pendingMuyaContent` mis à jour à chaque frappe ; un timer de 1500 ms tente de flush mais re-defère si encore focus ; un `onblur` flush immédiatement (pas de risque puisque le textarea n'est plus focus). Conséquence : la preview ne se met à jour que quand l'utilisateur fait une **pause >1.5 s** ou **change de pane**. Pas de live preview frame-par-frame, mais aucun defocus.
