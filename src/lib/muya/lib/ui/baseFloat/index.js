...
class BaseFloat {
  ...
  listen () {
    ...
    eventCenter.attachDOMEvent(document, 'click', (event) => {
      // Don't hide if the click originated from inside any float wrapper
      if (event.target && event.target.closest && event.target.closest('.ag-float-wrapper')) return
      // Don't hide if the click originated from QuickInsert (table picker opens from there)
      if (event.target && event.target.closest && event.target.closest('.ag-quick-insert')) return
      this.hide()
    })
