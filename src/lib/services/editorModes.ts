...
import { type Preferences, preferences } from '$lib/stores/preferences';
...
class EditorModes {
	...
	readonly state = derived(
		[preferences, editorStore.activeTab],
		([p, tab]): EditorState => ({
			...
			editorStore.activeTab.subscribe((tab) => {
				...
				if (readOnly && !prevReadOnly && muyaService.isReady()) {
					try {
						...
						console.warn('[EditorModes] blur failed:', e);
					}
				}
	...
	toggleSource(): void {
		this.toggle('sourceCodeMode');
	}
	toggleSplit(): void {
		this.toggle('splitView');
	}
	toggleFocus(): void {
		this.toggle('focusMode');
	}
	toggleTypewriter(): void {
		this.toggle('typewriterMode');
	}
