...
class MuyaService {
	...
	setMarkdown(md: string): void {
		if (!this.muya) return;
		try {
			this.muya.setMarkdown(md);
		} catch (e) {
			dlog('muya', 'setMarkdown:', e);
		}
	}
	...
	getCursor(): unknown {
		if (!this.muya) return null;
		try {
			return this.muya.getCursor();
		} catch (e) {
			dlog('muya', 'getCursor:', e);
			return null;
		}
	}
	...
	setOptions(opts: Record<string, unknown>, silent?: boolean): void {
		if (!this.muya) return;
		try {
			this.muya.setOptions(opts, silent);
		} catch (e) {
			dlog('muya', 'setOptions:', e);
		}
	}
	...
	shiftHeadingUp(): void {
		if (!this.muya) return;
		try {
			(this.muya as unknown as { updateParagraph: (t: string) => void }).updateParagraph('upgrade heading');
		} catch (e) {
			dlog('muya', 'shiftHeadingUp:', e);
		}
	}
	...
	shiftHeadingDown(): void {
		if (!this.muya) return;
		try {
			(this.muya as unknown as { updateParagraph: (t: string) => void }).updateParagraph('degrade heading');
		} catch (e) {
			dlog('muya', 'shiftHeadingDown:', e);
		}
	}
	...
	resetToParagraph(): void {
		if (!this.muya) return;
		try {
			(this.muya as unknown as { updateParagraph: (t: string) => void }).updateParagraph('paragraph');
		} catch (e) {
			dlog('muya', 'resetToParagraph:', e);
		}
	}
