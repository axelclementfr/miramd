import type { DocumentStats, Tab, TocEntry } from '$lib/types/editor';
...
function createEditorStore() {
	...
	function addTab(path: string | null = null, name = 'Untitled.md', content = '') {
		const id = generateId();
	...
	function updateContent(id: string, content: string) {
		tabs.update((t) =>
			t.map((tab) => (tab.id === id ? { ...tab, content, isModified: content !== tab.savedContent } : tab)),
		);
	}
